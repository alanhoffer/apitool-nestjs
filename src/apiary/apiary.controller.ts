...
export class ApiaryController {
  ...
  @UseGuards(AuthGuard)
  @Get('all/count')
  async getApiaryAndHiveCounts(@Request() req): Promise<{ apiaryCount: number; hiveCount: number }> {
    try {
      const userId = req.user.sub;
      // Fetch counts for apiaries and hives from their respective services
      const apiaryCount = await this.apiaryService.countApiariesByUserId(userId);
      const hiveCount = await this.apiaryService.countHivesByUserId(userId);
      ...
      throw new HttpException('Failed to retrieve counts.', HttpStatus.INTERNAL_SERVER_ERROR);
    }
  }
